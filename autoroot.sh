...
perform_root() {
    ...
    payload="$(printf '{"sourceId":"%s","message":"[%s] Rooting complete. You may need to reboot for Homebrew Channel to appear.<br>%s","buttons":[%s]}' "${SRC_APPID}" "${SCRIPT_NAME}" "${message}" "${buttons}")"
...
tempdir_oncefile="${tempdir}/autoroot.once"
tmp_oncefile='/tmp/autoroot.once'

[ -e "${tempdir_oncefile}" -a -e "${tmp_oncefile}" ] && { log 'Script already executed'; exit 3; }

touch -- "${tempdir_oncefile}" "${tmp_oncefile}"
